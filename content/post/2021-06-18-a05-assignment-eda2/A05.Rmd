...
To answer these questions seattle reservation dataset is analyzed, which is publicly accessible data, available dataset on Kaggle(....https://www.kaggle.com/jessemostipak/hotel-booking-demand).
...
I created the following two functions to display the visualisation in order to     investigate several behaviors of repeat visitors.
...
```{r}
guestbehabar (hotel_data$is_canceled)
```
```{r}
hotel_data %>% 
  group_by(is_repeated_guest) %>% 
...
```{r}
guestbehabar(hotel_data$deposit_type)
```
...
```{r}
guestbehabar(factor(hotel_data$total_of_special_requests))
```
...
```{r}
guestbehabar(hotel_data$customer_type)
```
...
Logistic Regression shows an accuracy of 78%.
